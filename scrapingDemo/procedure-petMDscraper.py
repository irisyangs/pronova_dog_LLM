...
def save_content_to_files(url):
    ...
    os.makedirs('ScrapedFiles_petMD_procedures', exist_ok=True)
    
    # Save the text content to a txt file in the ScrapedFiles folder
    filepath = os.path.join('ScrapedFiles_petMD_procedures', filename)
    ...
    json_filename = 'sources_petMD_procedures.json'
...
def extract_hrefs_from_divs(url):
    ...
    if response.status_code == 200:
        ...
        hrefs = [href for href in hrefs if href[0:4] == '/dog']
        # dogs only
        return hrefs
    else:
        return f"Failed to retrieve the webpage. Status code: {response.status_code}"
